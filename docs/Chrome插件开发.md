...
1. 打开含题目的正常网页，点击“提取当前题目”。插件会识别页面中的全部可见题目，并显示题目选择器。
...
提取器是通用启发式实现，会优先识别单选、多选、判断题常见的题干与 `A/B/C/D` 选项。对于结构特殊的在线题库，应在 `extension/content/content-script.js` 中补充站点选择器，而不应通过页面脚本获得模型密钥。
