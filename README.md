...
配合 Chrome 插件使用：在插件侧边栏「题库采集」小节开启开关，之后在任意已授权的题目页面按 **Alt / ⌥ + Shift + C**，当前页全部题目即一键入库（工具栏图标闪 ✓/! 反馈结果）。快捷键走浏览器级命令，与系统冲突时可在 `chrome://extensions/shortcuts` 自行修改。每次提取保存为 `data/extractions/` 下的一个独立 JSON 文件，重复提取全部保留（附内容哈希）。
