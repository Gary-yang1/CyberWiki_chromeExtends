...
启动后手机/电脑浏览器打开 <http://127.0.0.1:8790> 即可使用移动端题库界面（浏览提取记录、检索定位、调用 AI 解答、配置模型）。真机访问用 `--host 0.0.0.0` 启动，手机连同一局域网访问电脑 IP。

**多用户鉴权（可选）**：创建 `data/collector_auth.json` 即启用，采集内容按 User ID 分目录保存（`data/extractions/<userId>/`），所有 API 与网页都需凭据访问；删除该文件或留空 users 则回到单空间开放模式：

```json
{ "users": { "gary": "key-gary-xxxx", "alice": "key-alice-yyyy" } }
```

启用后：网页首次打开会出现登录页；插件侧边栏「题库采集」小节填写 User ID 和 Key，「测试采集服务连接」会一并校验凭据。

配合 Chrome 插件使用：在插件侧边栏「题库采集」小节开启开关，之后在任意已授权的题目页面按 **Alt / ⌥ + Shift + C**，当前页全部题目即一键入库（工具栏图标闪 ✓/! 反馈结果）。快捷键走浏览器级命令，与系统冲突时可在 `chrome://extensions/shortcuts` 自行修改。每次提取保存为一个独立 JSON 文件，重复提取全部保留（附内容哈希）。题库页顶部搜索框可按题干/选项/答案关键词检索，结果点击直达对应题目（定位高亮）。
