...
4. 点底部「**保存答题设置**」。
5. 在任意已授权的题目页面按 **`Alt / ⌥ + Shift + C`**：当前页全部题目一键发往题库。工具栏图标反馈：蓝色 `…` 提取进行中（大页面会停留数秒）；绿色 `✓` 上报成功；黄色 `?` 页面没有识别到题目；红色 `!` 网络/服务/权限错误。
...
- **详情**：题目分页浏览（每页 20 题，支持上一页/下一页与**题号直达**跳转并高亮定位）；「AI 解答」单题作答、「解答本页」按当前页批量作答、「全部解答 · 逐页」从第一页起逐页分批解答（按钮变「停止」可随时中断）；「删除」需二次确认（再次点击确认，4 秒后自动复位）。答案、置信度、模型、耗时写回文件持久保存。
